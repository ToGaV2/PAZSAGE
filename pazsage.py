...
class DocumentProcessor:
    ...
    def process(self):
        ...
        try:
            ...
            for i in range(len(items)):
                ...
                for j in range(len(items[i])):
                    ...
                    if (items[i][j].startswith('L1  - ')) or (items[i][j].startswith('L2  - ')):
                        ...
                        print(f"Generate Summary for {titleofpaper}")
                        try:
            ...
            self.root.after(0, self.update_ui, "", "", f"Error in Summary Build Section: {e}")
        ...
        try:
            for j in os.listdir(os.path.join(os.getcwd(),'summaries')):
                if not j.startswith('.'):
                    print(j)
                    with open(os.path.join(os.getcwd(),'summaries',j), 'r', encoding='utf-8') as f:
                        text = f.read()
            ...
            self.root.after(0, self.update_ui, "", "", f"Audio Build Error: {e}")
        ...
        try:
            ...
            self.root.after(0, self.update_ui, "", "", f"Zipping Error: {e}")
